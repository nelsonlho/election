...
const NV_MING_DAY: Record<
  string,
  { binlang: string[]; sansha: string; geshan: string[] }
> = {
  申子辰: {
    binlang: ['卯', '辰', '巳'],
    sansha: '未',
...
function jiXiongShen(
  info: DayInfo,
  mingGan: string | undefined,
  mingZhi: string,
  label = '本命',
): Reason[] {
  const out: Reason[] = [];
  ...
  if (XING_PAIRS.some(([a, b]) => a === mingZhi && b === dz))
    out.push({
      kind: '注',
      text: `日支刑${label}，慎用（原書：吉凶神定局列刑為忌）`,
    });
  return out;
}
...
const JU_TAI: Record<string, string> = {
  火: '子',
  木: '酉',
  金: '卯',
  水: '午',
...
const JU_MU: Record<string, string> = {
  火: '戌',
  木: '未',
  金: '丑',
  水: '辰',
...
function hunShenSha(
  ...
  if (fGan) {
    ...
    if (info.dayGanZhi === taiGZ)
      out.push({
        kind: '凶',
        text: `${taiGZ}日真胎元，大凶（原書：六十女總局）`,
      });
    else if (dz === taiZhi)
      out.push({
        kind: '注',
        text: `日臨胎元（胎元${taiGZ}），慎用——忌真胎元日（原書：六十女總局）`,
      });
  ...
  if (dz === zhiAdd(fZhi, 1)) {
    if (fGan && dg === ganAdd(fGan, 3))
      out.push({
        kind: '凶',
        text: `${info.dayGanZhi}日正沖夫星，大凶（原書：六十女總局）`,
      });
  ...
  if (fGan) {
    ...
    if (ts)
      out.push({
        kind: ts.kind,
        text: ts.text.replace('安牀大忌', '婚事大忌'),
      });
  }
  ...
  if (dz === zhiAdd(fZhi, 1))
    out.push({
      kind: '注',
      text: '犯殺翁——家有翁者忌：三德逢一可解，無德則新娘進門時翁少避；無翁不忌（原書：六十女總局）',
    });
  if (dz === zhiAdd(fZhi, 7))
    out.push({
      kind: '注',
      text: '犯殺姑——家有姑者忌：三德逢一可解，無德則新娘進門時姑少避；無姑不忌（原書：六十女總局）',
    });
  // 桃花（咸池）
  if (XIAN_CHI[fZhi] === dz)
    out.push({
      kind: '注',
      text: '犯桃花（咸池），慎用（原書：四柱有堆長生、進長生可解）',
    });
  // 天狗
  ...
  if (fGan) {
    ...
    else if (dz === WX_SI[guan])
      out.push({ kind: '注', text: '犯夫星死，慎用（原書：六十女總局左局）' });
    else if (dz === WX_JUE[guan])
      out.push({ kind: '注', text: '犯夫星絕，慎用（原書：六十女總局左局）' });
    if (dz === WX_MU[si])
      out.push({ kind: '注', text: '犯天嗣墓，慎用（原書：六十女總局左局）' });
    else if (dz === WX_SI[si])
      out.push({ kind: '注', text: '犯天嗣死，慎用（原書：六十女總局左局）' });
    else if (dz === WX_JUE[si])
      out.push({ kind: '注', text: '犯天嗣絕，慎用（原書：六十女總局左局）' });
  }
  // 孤辰・寡宿
  const { gu, gua } = guChenGuaSu(fZhi);
  if (dz === gu)
    out.push({ kind: '注', text: '犯孤辰，慎用（原書：六十女總局左局）' });
  if (dz === gua)
    out.push({ kind: '注', text: '犯寡宿，慎用（原書：六十女總局左局）' });
  // 箭刃（羊刃＋飛刃全）
  ...
  if (fGan && GAN_ORDER.indexOf(fGan) % 2 === GAN_ORDER.indexOf(dg) % 2)
    out.push({
      kind: '注',
      text: '日干與女命同陰陽（四柱六神值偏神），力弱慎用（原書：六十女總局頭欄）',
    });
  // 命宮盤沖（原書 108 頁）
  if (fGan) out.push(...gongChong(fGan, dz));
  // 紅鸞、天喜（原書 196 頁；註引會海：天喜乃血光之神，紅鸞非吉曜，逢吉神則吉、凶神則凶）
  const hongLuan =
    ZHI_ORDER_E[(((3 - ZHI_ORDER_E.indexOf(fZhi)) % 12) + 12) % 12];
  if (dz === hongLuan)
    out.push({
      kind: '注',
      text: '紅鸞日——原書：紅鸞非吉曜之物，逢吉神則吉、凶神則凶，並參餘法',
    });
  if (dz === ZHI_CHONG[hongLuan])
    out.push({
      kind: '注',
      text: '天喜日——原書：天喜乃血光之神，逢吉神則吉、凶神則凶，並參餘法',
    });
  return out;
}
...
function famuJi(info: DayInfo): Reason[] {
  ...
  if (dz === LU_BAN_JI[ji])
    out.push({
      kind: '凶',
      text: '魯班殺日，伐木架馬做梁大凶不用（原書第六期書326）',
    });
  if (dz === FU_SHA_JI[ji])
    out.push({
      kind: '凶',
      text: '斧殺日，伐木架馬大凶不用（原書第六期書326）',
    });
  if (dz === DAO_ZHEN_FM[ji])
    out.push({
      kind: '凶',
      text: '刀砧殺日，伐木架馬大凶不用（原書第六期書326）',
    });
  if (dz === MU_MA_FM[i])
    out.push({ kind: '凶', text: '木馬殺日，架馬不用（原書第六期書326）' });
  if (dz === TIAN_HUO_ZL[i % 4])
    out.push({ kind: '凶', text: '天火日，做梁不用（原書第六期書326）' });
  if (dz === TIAN_ZEI_FM[i])
    out.push({
      kind: '注',
      text: '天賊日，伐木忌之，宿時（得明星）或蔞時制之則吉（原書第六期書326）',
    });
  if (dz === LONG_HU[i])
    out.push({
      kind: '注',
      text: '龍虎日，入山伐木忌之，吉多可用（原書第六期書327）',
    });
  if (dz === SHAN_GE[i % 6])
    out.push({
      kind: '注',
      text: '山隔日，入山伐木忌之，吉多可用（原書第六期書327）',
    });
  if (ZHU_QUE_XUN.has(info.dayGanZhi))
    out.push({
      kind: '注',
      text: '朱雀巽日，入山忌之，鳳凰符制之則吉（原書第六期書327）',
    });
  return out;
}
...
function ruZhaiZhouTang(
  lunarDay: number,
  monthDayCount: number,
): { name: string; bad: boolean } {
  const idx =
    monthDayCount >= 30
      ? (lunarDay - 1) % 16
...
// ── 共同凶煞 ──────────────────────────────────────────────
// 各神煞所忌之事類
const WANG_WANG_EVENTS: EventKey[] = [
  'chuxing',
  'jiaqu',
  'ruzhai',
  'furen',
...
const SAN_SANG_EVENTS: EventKey[] = [
  ...ZANG_EVENTS,
  'chengfu',
  'chufu',
  'dongtu',
...
function commonBad(info: DayInfo, event: EventKey): Reason[] {
  ...
  if (s.siLi)
    out.push({
      kind: '凶',
      text: '四離日（二分二至前一日），氣序分離，忌用事',
    });
  if (s.siJue)
    out.push({ kind: '凶', text: '四絕日（四立前一日），氣序絕滅，忌用事' });
  if (s.yangGong) out.push({ kind: '凶', text: '楊公忌日，百事忌用' });
  if (s.yueJi)
    out.push({
      kind: '注',
      text: '月忌日（初五、十四、廿三），俗忌百事，吉多可用（原書第六期書329）',
    });
  if (s.jueYanHuo && JUE_YAN_EVENTS.includes(event))
    out.push({
      kind: '凶',
      text: '絕煙火日，忌入宅、移徙、作灶、出火（原書第六期書324・329 避宅出火忌例）',
    });
  if (
    LI_CHAO_EVENTS.includes(event) &&
    LI_CHAO[MONTH_ORDER_E.indexOf(info.monthZhi)] === info.dayZhi
  )
    out.push({
      kind: '注',
      text: '離巢日，入宅出火移徙忌之，吉多可用（原書第六期書328 避宅出火忌例）',
    });
  if (event === 'shangliang' && info.lunarDay === 9)
    out.push({
      kind: '凶',
      text: '橫天日（每月初九），忌上樑（原書第六期書325）',
    });
  if (event === 'yixi' && info.lunarDay === 25)
    out.push({
      kind: '凶',
      text: '橫天日（每月廿五），忌移徙（原書第六期書325）',
    });
  ...
  if (
    ZANG_EVENTS.includes(event) &&
    (info.dayZhi === '巳' || info.dayZhi === '亥')
  )
    out.push({ kind: '凶', text: '重日（巳亥日），葬事忌之，恐犯重喪' });
  if (ZANG_EVENTS.includes(event) && CHONG_SANG[info.monthZhi] === info.dayGan)
    out.push({
      kind: '凶',
      text: `重喪日（${info.monthZhi}月${info.dayGan}日），葬事大忌（原書：安葬日家凶神訣）`,
    });
  if (
    SAN_SANG_EVENTS.includes(event) &&
    SAN_SANG_DAY[info.monthZhi] === info.dayZhi
  )
    out.push({
      kind: '凶',
      text: `三喪日（四季之殺：春辰、夏未、秋戌、冬丑），喪葬動土犯是不用（原書：入殮成服動土忌例）`,
    });
  if (
    SAN_SANG_EVENTS.includes(event) &&
    FU_RI[info.monthZhi] === info.dayGan &&
    CHONG_SANG[info.monthZhi] !== info.dayGan
  )
    out.push({
      kind: '凶',
      text: `復日（${info.monthZhi}月${info.dayGan}日），喪葬忌之，恐致重喪（原書：安葬入殮成服忌例）`,
    });
  if (event === 'anzang') {
    if (JI_YUE.includes(info.monthZhi) && info.dayZhi === '丑')
      out.push({
        kind: '凶',
        text: '正紅紗（季月丑日），安葬忌之（原書：安葬忌例）',
      });
    if (info.lunarDay === 17)
      out.push({
        kind: '凶',
        text: '橫天朱雀（每月十七日），忌安葬（原書：安葬忌例）',
      });
...
function jianChuReason(info: DayInfo, event: EventKey): Reason | null {
  ...
  if (bad.includes(name))
    return {
      kind: '凶',
      text: `${name}日（建除）：${jc.ji.includes(EVENT_NAMES[event]) ? `忌${EVENT_NAMES[event]}` : `於${EVENT_NAMES[event]}不利`}。${jc.meaning}`,
    };
...
export const RULE_LAYERS: RuleLayer[] = [
  {
    key: 'yuejia',
    name: '月家神煞',
    desc: '月破、受死、往亡、歸忌、紅沙、四離四絕、楊公忌、月忌、絕煙火、重日、重喪、三喪',
  },
  {
    key: 'jianchu',
    name: '建除十二神',
    desc: '建滿平收黑，除危定執黃，成開可用，破閉不用',
  },
  { key: 'tongshu', name: '通書宜忌', desc: '逐日通書宜忌對照' },
  {
    key: 'pengzu',
    name: '彭祖百忌',
    desc: '亥不行嫁、申不安牀、巳不遠行、寅不祭祀等',
  },
  {
    key: 'chongming',
    name: '沖命破碎',
    desc: '日支正沖本命、破碎日（須入生年）',
  },
  {
    key: 'jixiong',
    name: '吉凶神定局',
    desc: '三合、六合、堆貴、進貴、堆祿、進祿、堆馬、刑（須入生年）',
  },
  {
    key: 'liyue',
    name: '行嫁利月',
    desc: '女命大利、小利、妨翁姑、妨父母、妨夫、妨婦月（論節氣）',
    events: ['jiaqu'],
  },
  {
    key: 'zhoutang',
    name: '嫁娶周堂',
    desc: '大月起夫順行、小月起婦逆行，值夫婦大凶',
    events: ['jiaqu'],
  },
  {
    key: 'nvming',
    name: '女命日吉凶',
    desc: '正檳榔殺、檳榔三殺、盤隔山殺、清吉取用（原書 86-89）',
    events: HUN_EVENTS,
  },
  {
    key: 'bujiang',
    name: '陰陽不將',
    desc: '不將大吉；月厭、厭對、俱將忌（原書將神名目）',
    events: HUN_EVENTS,
  },
  {
    key: 'hunsha',
    name: '婚神煞',
    desc: '沖胎元、沖夫星、沖天嗣、殺翁殺姑、桃花、天狗（原書六十女總局）',
    events: HUN_EVENTS,
  },
  {
    key: 'caiyiji',
    name: '裁衣忌例',
    desc: '長星、短星、天賊、白虎、朱雀、正四廢（原書 92-93）',
    events: ['caiyi'],
  },
  {
    key: 'anchuang',
    name: '安牀忌例',
    desc: '臥尸、死別、醞巢、天賊、木馬、箭頭、刀砧、天嗣犯沖（原書 109-111）',
    events: ['anchuang'],
  },
  {
    key: 'tuwang',
    name: '土王用事',
    desc: '四立前十八日忌動土破土穿井修墳',
    events: ['dongtu', 'potu', 'juejing', 'xiufen', 'kaishengfen', 'qiji'],
  },
  {
    key: 'shan',
    name: '沖山三殺',
    desc: '日支沖座山、流年三殺占山（須入座山；造葬以墓之坐山論）',
    events: [
      'ruzhai',
      'dongtu',
      'xiuzao',
  ...
  },
  {
    key: 'dongtuji',
    name: '動土忌例',
    desc: '土符、土瘟、天瘟、重日、白虎朱雀、土公死忌、黃帝死（原書 548・第六期333）',
    events: ['dongtu'],
  },
  {
    key: 'famuji',
    name: '伐木架馬做梁忌例',
    desc: '魯班殺、斧殺、刀砧、木馬、天火做梁、天賊、龍虎、山隔（原書第六期326-327）',
    events: ['famu'],
  },
  {
    key: 'zhengong',
    name: '震宮殺',
    desc: '飛宮月家殺：八節起宮、五虎遁至呼聲泊宮，殺宮三山忌上樑（須入座山；原書第九期）',
    events: ['shangliang'],
  },
  {
    key: 'dayuejian',
    name: '大月建',
    desc: '飛宮月家殺：年支起宮、逆佈九宮至本月建，其三山大忌修方修造修灶（須入座山；原書第九期。訣註：凡別事不忌）',
    events: ['xiufang', 'zuozao', 'xiuzao'],
  },
  {
    key: 'zhaizhou',
    name: '入宅安香周堂',
    desc: '十六位／八位周堂環，值凶位忌；出火同忌二分二至（原書 459-460）',
    events: ['ruzhai', 'anxiang', 'chuhuo'],
  },
  {
    key: 'kaishizhou',
    name: '開市周堂',
    desc: '八位環：債木、爭訟值日忌（原書第七期圈點）',
    events: ['kaishi'],
  },
  {
    key: 'yanqin',
    name: '演禽宿曜',
    desc: '廿八宿值日吉凶（通行值日歌，非講義——原書 110 註者於宿忌持譏，故預設關）：吉宿十四凶宿十四；角宿忌葬、鬼宿反宜葬',
    events: undefined,
  },
  {
    key: 'doushou',
    name: '斗首化曜',
    desc: '山斗首五行對年月日干化氣：元辰廉子武財吉、貪官破鬼忌（爐傳斗首，非講義——原書七層斗首註云今已不用）。須入座山，預設關',
    events: [
      'ruzhai',
      'dongtu',
      'xiuzao',
  ...
  },
  {
    key: 'shashi',
    name: '殺師日',
    desc: '地師登山之忌（通書俗傳口訣：春辰戌、夏卯酉、秋丑未、冬子午；派別有異，非講義出）——預設關，主事地師者自開',
    events: [
      'anzang',
      'potu',
      'qizan',
  ...
  },
  {
    key: 'xianming',
    name: '仙命諸忌',
    desc: '日沖仙命、三殺、三刑、旬空（須入亡者生年——原書第八期）',
    events: [
      'anzang',
      'potu',
      'qizan',
  ...
  },
  {
    key: 'bazuo',
    name: '八座劍鋒',
    desc: '年家八座日勿用；八座方、劍鋒方占山制化權用（原書第八期安葬凶神年支表）',
    events: [
      'anzang',
      'potu',
      'qizan',
  ...
  },
  {
    key: 'diya',
    name: '地啞年例',
    desc: '流年逐月地啞日（八日一週期），俗以制重喪三喪之屬（原書第八期）',
    events: [
      'anzang',
      'potu',
      'qizan',
  ...
  },
  {
    key: 'dikong',
    name: '地空年例',
    desc: '流年逐月地空亡日（八日一週期），空亡凶日，金火日或三合可制（原書第八期，瑞成本 418-419）',
    events: [
      'anzang',
      'potu',
      'qizan',
  ...
  },
  {
    key: 'nianke',
    name: '年剋山家',
    desc: '山運納音（山之洪範五行墓庫、年干庫運遁）；年月日納音剋山運則忌，柱中生扶可制（須入座山；原書第十期造葬廿四山總局，瑞成本 487-535）',
    events: [
      'ruzhai',
      'dongtu',
      'xiuzao',
  ...
  },
  {
    key: 'shantou',
    name: '山頭日忌',
    desc: '逐山日級凶神：星曜殺、山方殺、冲丁殺、曜殺、文曲旬、流日太歲、消滅殺（須入座山；原書第十期造葬廿四山總局左頁，涵廿四山全（壬至酉第十期、辛戌乾亥第十一期））',
    events: [
      'ruzhai',
      'dongtu',
      'xiuzao',
  ...
  },
  {
    key: 'chuling',
    name: '除靈周堂',
    desc: '除靈值人（父母孫男婦女客婿）宜避、值亡可用（原書第十二期書593值例）',
    events: ['chufu'],
  },
  {
    key: 'shangguan',
  ...
  },
  {
    key: 'matui',
    name: '馬前炙退',
    desc: '流年驛馬前一辰之退煞占山，造葬歲內忌之，宜三合補助（須入座山；原書造葬廿四山年家方位。都天夾都歲堆黃俟後）',
    events: [
      'ruzhai',
      'dongtu',
      'xiuzao',
  ...
  },
  {
    key: 'huitou',
    name: '回頭貢殺箭刃',
    desc: '辰戌丑未命遇四柱三合全局殺之（不能制化）；命干箭刃雙全（原書 56-57，須入生年）',
  },
];
...
export function qiLinFangDetail(
  monthZhi: string,
): { ji: string; tianGou: string; zhaoXian: string[]; heXian: string } | null {
...
function yanQin(info: DayInfo, event: EventKey): Reason | null {
  ...
  if (xiu === '角' && zang)
    return {
      kind: '凶',
      text: `值日${qin}——角宿造作嫁娶吉而忌埋葬（通行值日歌，非講義）`,
    };
  if (xiu === '鬼' && zang)
    return {
      kind: '吉',
      text: `值日${qin}——鬼宿諸事凶而埋葬反吉（通行值日歌，非講義）`,
    };
...
function douShou(info: DayInfo, m: string): Reason[] {
  ...
  const yaos = pillars.map(([label, g]) => ({
    label,
    ...douShouYao(shanWx, HUA_QI[g]),
  }));
  const desc = yaos.map((y) => `${y.label}${y.name}`).join('・');
  const badCore = yaos.filter(
    (y) => (y.label === '年' || y.label === '日') && y.ji,
  );
  if (badCore.length > 0)
    return [
      {
        kind: '凶',
        text: `斗首化曜（${m}山屬${shanWx}）：${desc}——${badCore.map((y) => y.label + '柱' + y.name).join('、')}忌（爐傳斗首，非講義）`,
      },
    ];
  if (yaos.every((y) => !y.ji))
    return [
      {
        kind: '吉',
        text: `斗首化曜（${m}山屬${shanWx}）：${desc}，三柱皆吉曜（爐傳斗首，非講義）`,
      },
    ];
  return [
    {
      kind: '注',
      text: `斗首化曜（${m}山屬${shanWx}）：${desc}——月柱逢忌曜，慎（爐傳斗首，非講義）`,
    },
  ];
}
...
function yinFuJi(info: DayInfo, m: string): Reason | null {
  ...
  if (!pair) return null;
  const gans = [
    info.yearGanZhi.charAt(0),
    info.monthGanZhi.charAt(0),
    info.dayGan,
  ];
...
function zaoZuoShan(
  ...
  if (ZHI_CHONG[m]) {
    ...
    if (ZHI_CHONG[m] === info.dayZhi)
      out.push({
        kind: '凶',
        text: `日支${info.dayZhi}沖山（坐${m}山），造作葬事忌之（原書：沖山例）`,
      });
    if (sha.includes(m))
      out.push({
        kind: '凶',
        text: `流年${info.yearGanZhi}三殺占山（${m}山），歲內造作葬事大忌（原書：三殺例）`,
      });
    if (ZHI_CHONG[yearZhi] === m)
      out.push({
        kind: '凶',
        text: `歲破占山（流年${info.yearGanZhi}沖${m}山，即向太歲），歲內大忌（原書：安葬山家凶神訣）`,
      });
    if (zuoTaiSui && m === yearZhi)
      out.push({
        kind: '注',
        text: `坐太歲（${m}山即流年${info.yearGanZhi}之方）——古謂太歲可坐不可向，造葬修方從權慎用`,
      });
    ...
    if (info.dayGan === GAN_SHAN_CHONG[m])
      out.push({
        kind: '凶',
        text: `日干${info.dayGan}沖山（坐${m}山，${m}${GAN_SHAN_CHONG[m]}對沖），忌之（原書：沖山例）`,
      });
    if (GAN_SHAN_FANG[m]?.every((z) => sha.includes(z)))
      out.push({
        kind: '凶',
        text: `流年${info.yearGanZhi}三殺占方（${m}山附焉），歲內造作葬事大忌（原書：三殺例）`,
      });
    ...
    if (GUA_SHAN_CHONG[m].includes(info.dayZhi))
      out.push({
        kind: '凶',
        text: `日支${info.dayZhi}沖山（坐${m}山，對宮${GUA_SHAN_CHONG[m].join('')}之沖），忌之（原書：沖山例）`,
      });
  }
  // ── 月家凶神占山（原書第八期月家八條）──────────────────────
  ...
  if (ZHI_CHONG[m]) {
    if (ZHI_CHONG[m] === mz)
      out.push({
        kind: '凶',
        text: `沖山凶月（${mz}月建沖${m}山，如年之歲破），月內大凶不可用（原書：月家凶神）`,
      });
    if (mSha.includes(m))
      out.push({
        kind: '凶',
        text: `三殺凶月（${mz}月三殺占${m}山），月家之大殺不可用（原書：月家凶神）`,
      });
    if (zhiAdd(mz, 9) === m)
      out.push({
        kind: '凶',
        text: `八座凶月（${mz}月八座方占${m}山），難有制法從俗大忌（原書：月家凶神）`,
      });
  } else if (
    GAN_SHAN_CHONG[m] &&
    GAN_SHAN_FANG[m]?.every((z) => mSha.includes(z))
  ) {
    out.push({
      kind: '凶',
      text: `三殺凶月（${mz}月三殺占方，${m}山附焉），月內不可用（原書：月家凶神）`,
    });
  }
  ...
  if (
    ZHI_CHONG[m]
      ? dSha.includes(m)
      : GAN_SHAN_CHONG[m]
        ? GAN_SHAN_FANG[m]?.every((z) => dSha.includes(z))
        : false
  )
    out.push({
      kind: '凶',
      text: `日三殺占山（課中${info.dayZhi}字，殺${dSha.join('')}方，${m}山在焉），不能用（原書：三殺例）`,
    });
  {
    ...
    if (ci >= 0 && MOUNTAINS_24[(ci + 1) % 24] === m)
      out.push({
        kind: '凶',
        text: `劍鋒殺月（${mz}月劍鋒占${m}山），忌葬勿用為要——太陽、木星、木局可制（原書：月家凶神）`,
      });
  }
  ...
  if (jian) {
    const pillarZhi: [string, string][] = [
      ['年', yearZhi],
      ['月', mz],
      ['日', info.dayZhi],
    ];
    const pillarGan: [string, string][] = [
      ['年', info.yearGanZhi.charAt(0)],
      ['月', info.monthGanZhi.charAt(0)],
      ['日', info.dayGan],
    ];
    if (ZHI_CHONG[jian]) {
      ...
      for (const [label, z] of pillarZhi)
        if (ZHI_CHONG[jian] === z)
          out.push({
            kind: '凶',
            text: `${label}支${z}沖兼（坐${m}山兼${jian}），謂之沖兼凶（原書：沖山例）`,
          });
      // 三殺七山之隅：卦山兼入殺方
      if (GUA_SHAN_CHONG[m]) {
        for (const [label, z] of pillarZhi) {
          ...
          if (s.includes(jian))
            out.push({
              kind: '凶',
              text: `三殺占兼（${label}課${z}字殺${s.join('')}方，坐${m}山兼${jian}即七山之隅），不能用（原書：三殺例）`,
            });
        }
      }
      ...
      for (const [label, g] of pillarGan)
        if (GAN_SHAN_CHONG[jian] === g)
          out.push({
            kind: '凶',
            text: `${label}干${g}沖兼（坐${m}山兼${jian}，${jian}${GAN_SHAN_CHONG[jian]}對沖），謂之沖兼凶（原書：沖山例）`,
          });
      ...
      for (const [label, z] of pillarZhi)
        if (GUA_SHAN_CHONG[jian].includes(z))
          out.push({
            kind: '凶',
            text: `${label}支${z}沖兼（坐${m}山兼${jian}，對宮${GUA_SHAN_CHONG[jian].join('')}之沖），謂之沖兼凶（原書：沖山例）`,
          });
    }
  }
  // 陰府（諸山皆判）
...
function xianMingJi(
  ...
  if (xGan && TIAN_YI[xGan]?.includes(dz))
    out.push({
      kind: '吉',
      text: `日支${dz}為仙命${xGan}干天乙貴人（陰宅貴人臨日）`,
    });
  if (ZHI_CHONG[xZhi] === dz)
    out.push({
      kind: '凶',
      text: `日支${dz}沖仙命（亡命${xZhi}），葬課大凶（原書：仙命諸忌例）`,
    });
  if (SAN_SHA_FANG[xZhi]?.includes(dz))
    out.push({
      kind: '凶',
      text: `仙命三殺日（${xZhi}命見${dz}日）——原書：真三殺大凶，非真者取祿馬貴人解化`,
    });
  if (XING_MAP.some(([a, b]) => a === xZhi && b === dz))
    out.push({
      kind: '注',
      text: '日支刑仙命（三刑凶日），宜取貴人解化從權（原書：仙命諸忌例）',
    });
  // 六甲旬空：仙命所在旬之空亡支
  if (xGan) {
    ...
    if (gi >= 0 && zi >= 0) {
      ...
      if (n < 60) {
        const head = n - (n % 10);
        const kong = [
          ZHI_ORDER_E[(head + 10) % 12],
          ZHI_ORDER_E[(head + 11) % 12],
        ];
        if (kong.includes(dz))
          out.push({
            kind: '注',
            text: `仙命旬空日（${kong.join('、')}為空亡），冷地空亡之屬，葬課慎用`,
          });
      }
    }
    ...
    if (gi2 >= 0) {
      ...
      if (info.dayGanZhi === '庚' + rentiZhi)
        out.push({
          kind: '注',
          text: `人梯地空亡日（仙命${xGan}干，庚${rentiZhi}日），空亡之屬，葬課慎用（原書：六十仙命諸空亡）`,
        });
    }
  }
  return out;
...
export function yinTaiYangQi(
  birthGan: string,
  month: number,
): { gan: string; zhi: string } | null {
...
function nianKeShanJia(info: DayInfo, mountain: string): Reason[] {
  ...
  const zhi = pillars.filter(
    ([, gz]) => naYinWx(gz) === w || WX_SHENG[naYinWx(gz)] === w,
  ); // 生扶山運
  const kePillars = ke
    .map(([n, gz]) => `${n}柱${gz}(${keNeeded}納音)`)
    .join('、');
  if (zhi.length > 0)
    return [
      {
        kind: '注',
        text: `年剋山家（${mountain}山山運納音屬${w}，${kePillars}剋山運）——柱中有${zhi.map(([n]) => n).join('')}柱納音生扶，可制權用（原書第十期造葬廿四山總局）`,
      },
    ];
  return [
    {
      kind: '凶',
      text: `年剋山家（${mountain}山山運納音屬${w}，${kePillars}剋山運，柱中無生扶之納音制化），造葬忌之（原書第十期造葬廿四山總局）`,
    },
  ];
}
...
function shanTouRiJi(info: DayInfo, mountain: string): Reason[] {
  ...
  if (gsc?.includes(info.dayZhi))
    out.push({
      kind: '凶',
      text: `冲山（${mountain}山忌${info.dayZhi}日，兼向本支之對沖），造葬大凶（原書造葬廿四山山頭凶神；干山之支級沖，別於日干沖）`,
    });
  const js = JIAN_SHE[mountain];
  if (
    js &&
    (js.unit === '支'
      ? js.pair.includes(info.dayZhi)
      : js.pair.includes(info.dayGan))
  )
    out.push({
      kind: '凶',
      text: `箭射（${mountain}山忌${js.pair.join('')}${js.unit}日之弓箭煞），造葬忌之（原書造葬廿四山山頭凶神）`,
    });
  if (d.xingyao.includes(gz))
    out.push({
      kind: '凶',
      text: `星曜殺（${mountain}山忌${gz}日，剋山正體五行之曜），日犯大忌、時犯小忌（原書造葬廿四山山頭凶神）`,
    });
  if (d.shanfang.includes(gz))
    out.push({
      kind: '凶',
      text: `山方殺（${mountain}山忌${gz}日），日犯大忌、時犯小忌，吉難抵制（原書造葬廿四山山頭凶神）`,
    });
  if (d.chongding.includes(gz))
    out.push({
      kind: '凶',
      text: `冲丁殺（${mountain}山忌${gz}日，即冲分金），修方更重、造葬亦凶（原書造葬廿四山山頭凶神）`,
    });
  if (d.yao === gz)
    out.push({
      kind: '凶',
      text: `曜殺（${mountain}山八卦曜殺${gz}日），日犯大忌、時犯小忌，吉不抵制（原書造葬廿四山山頭凶神）`,
    });
  ...
  if (xi >= 0) {
    ...
    if (d.wenqu.includes(xunHead))
      out.push({
        kind: '注',
        text: `文曲逢${xunHead}旬（${mountain}山，旬內十日），支山如犯須辦陰陽、金火填實（原書造葬廿四山山頭凶神）`,
      });
  }
  if (d.liuri === gz)
    out.push({
      kind: '凶',
      text: `流日太歲（${mountain}山值${gz}日），太陽到山或木局取用可解（原書造葬廿四山山頭凶神；原標旬，此取值日）`,
    });
  for (const [jq, g] of d.xiaomie) {
    if (JIE_QI_YUE[jq] === info.monthZhi && gz === g)
      out.push({
        kind: '凶',
        text: `消滅殺（${mountain}山，${jq}節內逢${g}日），造葬立向、修方大凶（原書造葬廿四山山頭凶神）`,
      });
  }
  return out;
}
...
function delingZhouTang(lunarDay: number): Reason[] {
  ...
  if (pos === '亡')
    return [
      {
        kind: '注',
        text: `除靈周堂值亡（農曆${lunarDay}日），此位可用（原書第十二期除靈周堂局）`,
      },
    ];
  return [
    {
      kind: '凶',
      text: `除靈周堂值${pos}（值生人之位），除靈宜避（原書第十二期除靈周堂局：如值人宜避）`,
    },
  ];
}
...
function shangGuanSha(info: DayInfo): Reason[] {
  ...
  for (const name of Object.keys(SHANG_GUAN_SHA))
    if (SHANG_GUAN_SHA[name][m] === info.dayZhi)
      out.push({
        kind: '凶',
        text: `${name}日（${info.monthZhi}月忌${info.dayZhi}日，小忌），赴任上官忌之（原書第十二期上官日凶神）`,
      });
  return out;
}
...
function zaoChuanSha(info: DayInfo): Reason[] {
  ...
  for (const name of Object.keys(ZAO_CHUAN_SHA)) {
    ...
    if (arr[m] === info.dayZhi)
      out.push({
        kind: '凶',
        text: `${name}日（${info.monthZhi}月忌${info.dayZhi}日，${note}），造船忌之（原書第十二期造船日凶神）`,
      });
  }
  return out;
}
...
function qiFuJi(info: DayInfo): Reason[] {
  ...
  for (const [m, d, name] of QIFU_DATE)
    if (info.lunarMonth === m && info.lunarDay === d)
      out.push({
        kind: '吉',
        text: `祈福吉日（${name}），宜齋醮祈福（原書第十二期祈福章）`,
      });
  if (info.lunarDay === 7)
    out.push({
      kind: '吉',
      text: `祈福吉日（每月初七北斗辰會），宜祈福（原書第十二期四始吉日）`,
    });
  const g = QIFU_GZ[info.dayGanZhi];
  if (g)
    out.push({
      kind: '吉',
      text: `祈福吉日（${info.dayGanZhi}日${g}），宜齋醮祈福（原書第十二期）`,
    });
  return out;
}

// ── 馬前炙退（原書第十/十一期造葬廿四山年家方位；逐流年占一山）───────────────
// 炙退＝驛馬前一辰（順一位）。三合局驛馬：申子辰馬寅、巳酉丑馬亥、寅午戌馬申、亥卯未馬巳。
// 炙退：申子辰年卯、巳酉丑年子、寅午戌年酉、亥卯未年午。占此支之山，歲內造葬忌之。
...
const GUA_SHAN_ZHI: Record<string, string[]> = {
  乾: ['戌', '亥'],
  坤: ['未', '申'],
  艮: ['丑', '寅'],
  巽: ['辰', '巳'],
};
// 座山所占之支（方位煞用）：支山本支、干山方三支、卦山本宮二支
function shanZhiSet(m: string): string[] {
  if (ZHI_ORDER_E.includes(m)) return [m];
  if (GAN_SHAN_FANG[m]) return GAN_SHAN_FANG[m];
  if (GUA_SHAN_ZHI[m]) return GUA_SHAN_ZHI[m];
  return [];
}
function maQianZhiTui(info: DayInfo, mountain: string): Reason[] {
  const ma = YI_MA[info.yearGanZhi.charAt(1)];
  if (!ma) return [];
  const zt = zhiAdd(ma, 1); // 馬前一辰
  if (shanZhiSet(mountain).includes(zt))
    return [
      {
        kind: '凶',
        text: `馬前炙退（流年${info.yearGanZhi}炙退占${zt}方，坐${mountain}山在焉），歲內造葬忌之——宜三合大進、補助令吉（原書造葬廿四山年家方位）`,
      },
    ];
  return [];
}
...
function baZuoJi(
  ...
  if (BA_ZUO_DAY[yz] === info.dayGanZhi)
    out.push({
      kind: '凶',
      text: `八座日（${yz}年${info.dayGanZhi}日），葬課勿用（原書：安葬凶神年支表；訣云「橫天八座不堪留」）`,
    });
  if (m) {
    ...
    if (m === baFang)
      out.push({
        kind: '注',
        text: `八座凶方占山（${yz}年${baFang}方），制化權用（原書：安葬凶神年支表）`,
      });
    ...
    if (m === jianFeng)
      out.push({
        kind: '注',
        text: `劍鋒凶方占山（${yz}年${jianFeng}方），制化權用（原書：安葬凶神年支表）`,
      });
  }
  return out;
}
...
export function evaluateDay(
  ...
  if (on('pengzu') && event === 'anchuang' && info.dayZhi === '申')
    reasons.push({
      kind: '凶',
      text: '彭祖百忌：申不安牀，鬼祟入房（原書：十二月皆忌申日）',
    });
  ...
  if (event === 'anchuang' && on('anchuang')) {
    ...
    if (opts.birthZhi && info.dayZhi === zhiAdd(opts.birthZhi, 1)) {
      if (opts.birthGan && info.dayGan === ganAdd(opts.birthGan, 3))
        reasons.push({
          kind: '凶',
          text: `${info.dayGanZhi}日正沖夫星，安牀大忌（原書：安床碎金賦）`,
        });
      else
        reasons.push({
          kind: '注',
          text: '日支沖夫星，安牀慎用（原書：安床碎金賦）',
        });
    }
    ...
    if (opts.femaleBirthGan && opts.femaleBirthMonth) {
      ...
      if (t && ZHI_CHONG[t.zhi] === info.dayZhi)
        reasons.push({
          kind: '凶',
          text: `日支${info.dayZhi}沖陰胎（坤造${opts.femaleBirthMonth}月生，陰胎${t.gan}${t.zhi}），安牀忌之（原書 107）`,
        });
    }
    if (opts.birthGan && opts.birthMonth) {
      ...
      if (t && ZHI_CHONG[t.zhi] === info.dayZhi)
        reasons.push({
          kind: '凶',
          text: `日支${info.dayZhi}沖陽氣（乾造${opts.birthMonth}月生，陽氣${t.gan}${t.zhi}），安牀忌之（原書 107）`,
        });
    }
  }
  ...
  if (useFemale && opts.femaleBirthZhi)
    persons.push({
      label: `女命${opts.femaleBirthZhi}`,
      zhi: opts.femaleBirthZhi,
      gan: opts.femaleBirthGan,
    });
  if (opts.persons?.length) {
    for (const p of opts.persons)
      persons.push({
        label: p.year ? `${p.year}（${p.zhi}）命` : `本命${p.zhi}`,
        zhi: p.zhi,
        gan: p.gan,
      });
  } else if (!useFemale && opts.birthZhi) {
    persons.push({
      label: `本命${opts.birthZhi}`,
      zhi: opts.birthZhi,
      gan: opts.birthGan,
    });
  }
  for (const p of persons) {
    if (on('chongming') && isChongMing(p.zhi, info.dayZhi))
      reasons.push({
        kind: '凶',
        text: `日支${info.dayZhi}正沖${p.label}，犯沖大凶`,
      });
    // 破碎日：通書公例；嫁娶另依原書女命日吉凶表，不重複計
    if (on('chongming') && event !== 'jiaqu' && isPoSui(p.zhi, info.dayZhi))
      reasons.push({
        kind: '凶',
        text: `破碎日（${p.label}見${info.dayZhi}日），忌用`,
      });
    // 吉凶神定局（原書 59 頁起）
    if (on('jixiong'))
      reasons.push(...jiXiongShen(info, p.gan, p.zhi, p.label));
    ...
    if (on('huitou')) {
      ...
      if (need) {
        ...
        if (have.length === 3)
          reasons.push({
            kind: '凶',
            text: `年月日支${need.join('')}三合全局，${p.label}犯回頭貢殺，不能制化（原書 56）`,
          });
        else if (have.length === 2)
          reasons.push({
            kind: '注',
            text: `年月日支已見${have.join('')}二字，${p.label}擇時勿再取${need.filter((z) => !have.includes(z)).join('')}時，恐成回頭貢殺（原書 56）`,
          });
      }
      ...
      if (pair && pair.every((z) => pillars.includes(z)))
        reasons.push({
          kind: '凶',
          text: `課中${pair.join('')}雙全，${p.label}犯箭刃（原書 57：逢天乙貴人到、三合化刃則喜）`,
        });
    }
  }
  ...
  if (on('shashi')) {
    ...
    if (SHASHI_EVENTS.includes(event)) {
      ...
      if (pairs.includes(info.dayZhi))
        reasons.push({
          kind: '注',
          text: '殺師日（通書俗傳口訣，派別有異）——主事地師勿登山扶作，事主無妨',
        });
    }
  }
  ...
  if (
    on('tuwang') &&
    ['dongtu', 'potu', 'juejing', 'xiufen', 'kaishengfen', 'qiji'].includes(
      event,
    ) &&
    isTuWang(info.solar.y, info.solar.m, info.solar.d)
  )
    reasons.push({
      kind: '凶',
      text: '土王用事（四立前十八日，土旺），忌動土破土',
    });

  // 造作沖山、三殺（原書第六期）：入宅動土修造上樑，有座向則判
  if (on('shan') && ZAO_ZUO_EVENTS.includes(event) && opts.mountainZhi) {
    reasons.push(
      ...zaoZuoShan(info, opts.mountainZhi, on('zuotaisui'), opts.jianXiang),
    );
  }
  ...
  if (
    on('dayuejian') &&
    ['xiufang', 'zuozao', 'xiuzao'].includes(event) &&
    opts.mountainZhi
  ) {
  ...
  // 馬前炙退（原書造葬廿四山年家方位）：造葬有座山則判流年炙退占山
  if (
    on('matui') &&
    (ZAO_ZUO_EVENTS.includes(event) || ZANG_EVENTS.includes(event)) &&
    opts.mountainZhi
  ...
  if (
    ...
    reasons.push({
      kind: '吉',
      text: `流年${info.yearGanZhi.charAt(0)}干天乙貴人臨日（二宅通用日課貴人）`,
    });
  ...
  if (
    on('dikong') &&
    ZANG_EVENTS.includes(event) &&
    isDiKong(info.yearGanZhi.charAt(1), info.lunarMonth, info.lunarDay)
  ) {
    reasons.push({
      kind: '注',
      text: '地空日（原書：地空年例）——空亡凶日，惟金火日或三合可制權用（葬事）',
    });
  }
  if (
    on('diya') &&
    SAN_SANG_EVENTS.includes(event) &&
    isDiYa(info.yearGanZhi.charAt(1), info.lunarMonth, info.lunarDay)
  ) {
    reasons.push({
      kind: '注',
      text: '地啞日（原書：地啞年例）——俗以制重喪、三喪之屬，喪葬課可權用',
    });
  }
  ...
  if (on('kaishizhou') && event === 'kaishi') {
    ...
    if (zt.bad)
      reasons.push({
        kind: '凶',
        text: `開市周堂值「${zt.name}」（原書圈點凶位），忌用`,
      });
    else reasons.push({ kind: '吉', text: `開市周堂值「${zt.name}」，吉` });
  }

  // 入宅、安香周堂（原書 459-460 頁）；出火同忌二分二至
  if (
    on('zhaizhou') &&
    (event === 'ruzhai' || event === 'anxiang' || event === 'chuhuo')
  ) {
    const zt =
      event === 'chuhuo'
        ? null
    ...
    if (zt && zt.bad)
      reasons.push({
        kind: '凶',
        text: `${EVENT_NAMES[event]}周堂值「${zt.name}」（原書圈點凶位），忌用`,
      });
    else if (zt)
      reasons.push({
        kind: '吉',
        text: `${EVENT_NAMES[event]}周堂值「${zt.name}」，吉`,
      });
    // 歸火入宅忌例：二分二至之日忌
    if (['春分', '秋分', '夏至', '冬至'].includes(info.jieQi))
      reasons.push({
        kind: '凶',
        text: `二分二至日（${info.jieQi}），忌入宅歸火出火安香（原書：歸火入宅忌例）`,
      });
  }
  ...
  if (event === 'jiaqu') {
    if (on('liyue') && opts.femaleBirthZhi) {
      ...
      if (cat === '大利月')
        reasons.push({
          kind: '吉',
          text: `本月（${info.monthZhi}月建）為女命${opts.femaleBirthZhi}之大利月`,
        });
      else if (cat === '小利月')
        reasons.push({
          kind: '注',
          text: '本月為小利月，清吉合式可用（妨媒之說，原書斥為誣謬）',
        });
      else if (cat === '妨翁姑月')
        reasons.push(
          de.length > 0
            ...
            : {
                kind: '注',
                text: '本月妨翁姑，日無三德，新娘入門後三朝方登堂拜見翁姑',
              },
        );
      else if (cat === '妨父母月')
        reasons.push(
    ...
    if (on('zhoutang')) {
      ...
      reasons.push({
        kind: zt.ji,
        text: `嫁娶周堂值「${zt.name}」，原書：${zt.note}`,
      });
      reasons.push({
        kind: '注',
        text: `是日白虎值「${zt.baihu}」，麟符${zt.linfu === '免用' ? '免用' : `${zt.linfu}以制`}`,
      });
    }
  }
