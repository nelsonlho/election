...
function MountainSelect({
  ...
  const neighbors =
    mi >= 0 ? [MOUNTAINS_24[(mi + 23) % 24], MOUNTAINS_24[(mi + 1) % 24]] : [];
  return (
    <label className="block text-sm">
      <span className="mb-1 block font-medium">
        ...
        {value && MOUNTAIN_WX[value] && (
          <span className="ml-2 text-red-600 dark:text-red-400">
            {value}山屬{MOUNTAIN_WX[value]}（正體五行）
          </span>
        )}
      </span>
  ...
  return (
    <div className="mt-1.5 flex flex-wrap gap-1.5">
      {years.map((y, i) => (
        ...
        >
          {y}年　{yearGanOfBirthYear(y)}
          {yearZhiOfBirthYear(y)}命
          <button
            type="button"
...
function useDisabledLayers() {
  ...
  const toggle = (key: string) => {
    ...
    setRaw(
      JSON.stringify(
        off.includes(token) ? off.filter((k) => k !== token) : [...off, token],
      ),
    );
  };
...
function EventPicker({
  ...
  value: EventKey;
  onChange: (e: EventKey) => void;
}) {
  const currentCat =
    EVENT_CATEGORIES.find((c) => c.events.some((e) => e.key === value))
      ?.category ?? EVENT_CATEGORIES[0].category;
    ...
    <div className="rounded-lg border border-stone-200 bg-white p-4 shadow-sm dark:border-stone-700 dark:bg-stone-800">
      ...
      >
        <div>
          <div className="flex items-center gap-2">
            <span
              className={`rounded px-2 py-0.5 text-sm font-bold ${RATING_STYLE[evaluation.rating]}`}
            >
              {evaluation.rating}
            </span>
      ...
      {open && (
        <div className="mt-3 space-y-1.5 border-t border-stone-100 pt-3 text-sm text-stone-600 dark:border-stone-700 dark:text-stone-400">
          <div>
            干支：{info.yearGanZhi}年　{info.monthGanZhi}月　{info.dayGanZhi}日
          </div>
          ...
          <div>
            通書宜：
            <Terms list={info.yi} kind="宜" />
          </div>
          <div>
            通書忌：
            <Terms list={info.ji} kind="忌" />
          </div>
          <div>
            吉時：
            {goodHours.length === 0 ? (
      ...
      <div style={{ maxWidth: '42rem', margin: '0 auto', lineHeight: 1.7 }}>
        ...
        >
          <tbody>
            ...
            <tr>
              ...
              <td>
                {info.solar.y}年{info.solar.m}月{info.solar.d}日　星期
                {info.solar.week}
              </td>
            </tr>
            <tr>
        ...
        >
          {evaluation.reasons.map((r, i) => (
            <li key={i} style={{ padding: '0.1rem 0' }}>
              {REASON_MARK[r.kind] ?? ''}
              {r.text}
            </li>
          ))}
          {evaluation.reasons.length === 0 && (
            <li style={{ padding: '0.1rem 0' }}>是日無宜忌之神，平常。</li>
          )}
        </ul>
        <table
          style={{
              ...
              <td>
                {goodHours.length
                  ? goodHours
                      .map(
                        (h) =>
                          `${h.reasons.some((x) => x.text.includes('登天門')) ? '★' : ''}${h.ganZhi}時(${h.range})`,
                      )
                      .join('、')
                  : '是日無純吉之時'}
              </td>
...
function CalendarHeatmap({
  ...
  // 每月吉日數（導覽提示何月有吉）
  const jiCount = (dm: Map<number, DayResult>) =>
    [...dm.values()].filter((r) => r.evaluation.rating === '吉').length;
  const cellCls = (rating: Rating, sel: boolean) => {
    const base =
      'flex h-8 w-full items-center justify-center rounded text-xs transition-colors ';
    const ring = sel
      ? 'ring-2 ring-red-500 ring-offset-1 dark:ring-offset-stone-800 '
      : '';
    if (rating === '吉')
      return (
        base +
        ring +
        'cursor-pointer bg-red-600 font-semibold text-white hover:bg-red-500'
      );
    if (rating === '凶')
      return (
        base +
        ring +
        'cursor-pointer bg-stone-800 text-stone-400 hover:bg-stone-700 dark:bg-stone-950 dark:text-stone-500'
      );
    return (
      base +
      ring +
      'cursor-pointer bg-stone-200 text-stone-600 hover:bg-stone-300 dark:bg-stone-700 dark:text-stone-300'
    );
  };
  return (
    <div className="rounded-lg border border-stone-200 bg-white p-4 shadow-sm dark:border-stone-700 dark:bg-stone-800">
      <div className="mb-2 flex items-center gap-3 text-xs text-stone-500">
        <span className="flex items-center gap-1">
          <span className="inline-block h-3 w-3 rounded-sm bg-red-600" />吉
        </span>
        <span className="flex items-center gap-1">
          <span className="inline-block h-3 w-3 rounded-sm bg-stone-200 dark:bg-stone-700" />
          平
        </span>
        <span className="flex items-center gap-1">
          <span className="inline-block h-3 w-3 rounded-sm bg-stone-800 dark:bg-stone-950" />
          凶
        </span>
          ...
          return (
            <div>
              ...
              <div className="grid grid-cols-7 gap-1">
                {WEEK_HEAD.map((w) => (
                  <div key={w} className="text-center text-xs text-stone-400">
                    {w}
                  </div>
                ))}
                {cells.map((d, i) => {
                  ...
                  if (!r)
                    return (
                      <div
                        key={d}
                        className="flex h-8 items-center justify-center text-xs text-stone-300 dark:text-stone-600"
                      >
                        {d}
                      </div>
                    );
...
function SearchTab() {
  ...
  const run = () => {
    ...
    setResults(
      findAuspiciousDays({
        ...
        xianMingYear:
          isZang && /^\d{4}$/.test(xianMing) ? Number(xianMing) : undefined,
        femaleBirthMonth:
          event === 'anchuang' && femaleMonth ? Number(femaleMonth) : undefined,
        birthMonth:
          event === 'anchuang' && birthMonth ? Number(birthMonth) : undefined,
        disabledLayers: off,
      }),
    );
  };
  ...
  useEffect(() => {
    ...
    const set = (p: string, f: (v: string) => void) => {
      const v = sp.get(p);
      if (v !== null) f(v);
    };
    set('ev', (v) => {
      if (v in EVENT_NAMES) setEventStr(v);
    });
    set('f', setFemaleYear);
    set('b', setBirthYear);
    set('s', setStart);
    ...
    set('jx', setJianXiang);
    set('xm', setXianMing);
    set('fm', setFemaleMonth);
    set('bm', setBirthMonth);
    const of = sp.get('off');
    if (of !== null) setOff(of ? of.split(',') : []);
  ...
  const hourOpts = {
    ...
    xianMingZhi:
      isZang && /^\d{4}$/.test(xianMing)
        ? yearZhiOfBirthYear(Number(xianMing))
        : undefined,
    mountainZhi: isZaoZuo && mountain ? mountain : undefined,
    jianXiang: isZaoZuo && mountain && jianXiang ? jianXiang : undefined,
  };
  const selected = selKey
    ? ((results ?? []).find((r) => dayKey(r) === selKey) ?? null)
    : null;
  const pingN = (results ?? []).filter(
    (r) => r.evaluation.rating === '平',
  ).length;
  ...
  const mingSummary =
    [
      ming === 'female' && /^\d{4}$/.test(femaleYear)
        ? `女命${femaleYear}（${yearZhiOfBirthYear(Number(femaleYear))}）`
        : '',
      ...personsOfYears(parseYears(birthYear)).map(
        (p) => `${ming === 'female' ? '男方' : '本命'}${p.year}（${p.zhi}）`,
      ),
      isZang && /^\d{4}$/.test(xianMing)
        ? `仙命${xianMing}（${yearGanOfBirthYear(Number(xianMing))}${yearZhiOfBirthYear(Number(xianMing))}）`
        : '',
    ]
      .filter(Boolean)
      .join('、') || undefined;
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border border-stone-200 bg-white p-4 shadow-sm dark:border-stone-700 dark:bg-stone-800">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="block text-sm">
              ...
              <input
                className="w-full rounded border border-stone-300 bg-white px-3 py-2 dark:border-stone-600 dark:bg-stone-900"
                placeholder={
                  ming === 'female' ? '例：1996（男方）' : '例：1990、1965'
                }
                value={birthYear}
            ...
            {isZang && (
              <label className="block text-sm">
                <span className="mb-1 block font-medium">
                  ...
                  {/^\d{4}$/.test(xianMing) && (
                    <span className="ml-2 text-red-600 dark:text-red-400">
                      {yearGanOfBirthYear(Number(xianMing))}
                      {yearZhiOfBirthYear(Number(xianMing))}命
                    </span>
                  )}
                </span>
                <input
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="block text-sm">
              <span className="mb-1 flex items-center font-medium">
                ...
                <button
                  type="button"
                  className="ml-auto rounded-full bg-stone-100 px-2 py-0.5 text-xs font-normal text-stone-600 ring-1 ring-stone-200 transition-colors hover:bg-stone-200 dark:bg-stone-700 dark:text-stone-300 dark:ring-stone-600"
                  onClick={() => {
                    setStart(todayStr());
                    setDaysStr('90');
                  }}
        ...
        <details
          className="mt-3"
          open={off.length > 0 || (isZaoZuo && !!mountain)}
        >
          <summary className="cursor-pointer select-none text-sm text-stone-500 hover:text-stone-700 dark:hover:text-stone-300">
            進階選項
            {off.filter((k) => !k.startsWith('enable:')).length > 0 && (
              <span className="ml-2 text-red-600 dark:text-red-400">
                （停用{off.filter((k) => !k.startsWith('enable:')).length}層）
              </span>
            )}
          </summary>
            ...
            <div>
              <div className="mb-1.5 flex items-center justify-between text-sm text-stone-500">
                ...
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    className="inline-flex items-center gap-1 rounded-lg border border-stone-300 px-3 py-1.5 text-stone-600 hover:bg-stone-100 dark:border-stone-600 dark:text-stone-300 dark:hover:bg-stone-700"
...
function DayTab() {
  ...
  const shareUrl = () =>
    buildShareUrl({
      tab: 'day',
      s: date,
      cat: dayCat === '全部' ? undefined : dayCat,
      f: femaleYear,
      b: birthYear,
      xm: xianMing,
      fm: femaleMonth,
  ...
  const showMountain =
    dayCat === '全部' || dayCat === '居家造作' || dayCat === '造葬';

  const results = useMemo(() => {
    const [y, m, d] = date.split('-').map(Number);
    if (!y || !m || !d) return null;
    const fy =
      showFemale && /^\d{4}$/.test(femaleYear) ? Number(femaleYear) : undefined;
    const xy =
      showXian && /^\d{4}$/.test(xianMing) ? Number(xianMing) : undefined;
    ...
    return (Object.keys(EVENT_NAMES) as EventKey[]).map((ev) =>
      queryDay(y, m, d, ev, {
        ...
        jianXiang:
          showMountain && mountain && jianXiang ? jianXiang : undefined,
        disabledLayers: off,
      }),
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
          ...
          {JIANCHU[info.zhiXing as keyof typeof JIANCHU] && (
            <div className="mt-3 rounded-lg border border-amber-300 bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-950/40">
              ...
              <p className="mt-1 text-sm">
                ...
                <span className="ml-4 font-medium text-stone-800 dark:text-stone-200">
                  忌
                </span>
      ...
      <div className="space-y-3">
        {results?.map((r, i) => {
          ...
          const cat = EVENT_CATEGORIES.find((c) =>
            c.events.some((e) => e.key === key),
          )?.category;
          if (dayCat !== '全部' && cat !== dayCat) return null;
          if (dayRating !== '全' && r.evaluation.rating !== dayRating)
            return null;
          return (
            <div
              key={i}
            ...
            >
              <div className="flex items-center gap-2">
                <span
                  className={`rounded px-2 py-0.5 text-sm font-bold ${RATING_STYLE[r.evaluation.rating]}`}
                >
                  {r.evaluation.rating}
                </span>
                <span className="text-lg font-semibold">
      ...
      {[5, 4, 3, 2, 1, 0].map((i) => {
        ...
        return (
          <div key={i} className="flex items-center gap-3">
            <div
              className={`flex w-20 shrink-0 gap-1.5 ${special ? 'text-amber-600 dark:text-amber-400' : 'text-stone-800 dark:text-stone-200'}`}
            >
              {lines[i] ? (
                ...
                <>
                  <span
                    className="h-2 w-1/2 rounded-sm bg-current"
            ...
            {yaos.length > 0 && (
              <>
                <span className="w-9 shrink-0 text-xs text-stone-400">
                  {YAO_NAMES[i]}
                </span>
                {y && (
                  <span
                    className={`text-sm ${special ? 'font-medium text-amber-700 dark:text-amber-400' : 'text-stone-600 dark:text-stone-400'}`}
                  >
                    {y.ganZhi}　{y.liuQin}
                    {y.shiYing && (
...
function HehunTab() {
  ...
  useEffect(() => {
    ...
    const f = sp.get('f');
    if (f !== null) setFemaleYear(f);
    const m = sp.get('m');
    if (m !== null) setMaleYear(m);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  const result =
    fy && my
      ? heHun(
          yearGanOfBirthYear(fy),
          yearZhiOfBirthYear(fy),
          yearGanOfBirthYear(my),
          yearZhiOfBirthYear(my),
        )
      : null;
  return (
    <div className="space-y-4">
      ...
      {result && (
        <>
          ...
          >
            <div className="flex items-center gap-2">
              <span
                className={`rounded px-2 py-0.5 text-sm font-bold ${RATING_STYLE[result.verdict]}`}
              >
                {result.verdict}
              </span>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-lg border border-stone-200 bg-white p-4 shadow-sm dark:border-stone-700 dark:bg-stone-800">
              ...
              <div className="mt-1 text-sm text-stone-600 dark:text-stone-400">
                {result.tiPalace}宮・{result.tiWuXing}　｜　女{result.femaleGua}
                上・男{result.maleGua}下 （男命泊{result.malePalace}）
              </div>
            </div>
            <div className="rounded-lg border border-stone-200 bg-white p-4 shadow-sm dark:border-stone-700 dark:bg-stone-800">
              ...
              <div className="mt-1 flex items-center gap-4">
                ...
                <HexagramFigure
                  upper={result.yongUpper}
                  lower={result.maleGua}
                  yaos={[]}
                />
              </div>
              <div className="mt-1 text-sm text-stone-600 dark:text-stone-400">
                {result.yongPalace}宮・{result.yongWuXing}　｜　女支
                {result.yongUpper}上・男{result.maleGua}下 （一卦管三山）
              </div>
            </div>
          </div>
          <div className="rounded-lg border border-stone-200 bg-white p-4 shadow-sm dark:border-stone-700 dark:bg-stone-800">
            <div className="text-sm font-medium">
              體卦六爻　
              <span className="font-serif text-base">{result.tiGua}</span>
              <span className="ml-2 text-xs font-normal text-stone-400">
  ...
  return (
    <div className="rounded-lg border border-stone-200 bg-white p-4 shadow-sm dark:border-stone-700 dark:bg-stone-800">
      <div className="text-sm font-medium">
        ...
        <span className="ml-2 text-xs font-normal text-stone-400">
          貴登天門為時家最吉（原書
          224）；命造欄有入則並判沖命、埋兒凶時；座山有入則並判時沖山、時三殺
        </span>
      </div>
        ...
        {hours.map((h) => (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <span
                className={`rounded px-1.5 text-xs font-bold ${RATING_STYLE[h.rating]}`}
              >
                {h.rating}
              </span>
            </div>
